...
- `planOnly` selects the Codex read-only sandbox and Claude Code plan permission mode. `inherit` does not override the shared thread and uses local defaults for new sessions. Only the separately confirmed `fullAccess` policy enables dangerous bypass flags; it grants allowlisted remote users unsandboxed execution.
